...
/**
* Prototype Vitel service mixin factory
*
* All services extend this base function result by default
* Its also recommended this is set when declaring custom services so that hot-reloading doesn't throw errors for anything they may depend on
*
* @param {Object} [options] Additional options to mutate behaviour
* @param {Boolean} [options.debug=false] Enable debugging for this mixin
...
*
* @returns {VueComponent} Prototype VueComponent mixin
*/
export function VitelServiceMixin(options) {
	let settings = {
		debug: false,
		debugLogger(...msg) {
			console.info(`[%c${settings.name}]`, 'font-weight:bold',
				...msg.map(v => isProxy(v) ? cloneDeep(v) : v) // Collapse proxies into clones
			);
		},
		...options,
	};
	...
	return {
		methods: {
			/**
			* Output debugging information if the debug flag is enabled for a service
			*
			* @param {Function|*...} [msg] Message components to output. If a function is given as the soel argument it is evaluated and the output given as the debug message. Async functions are waited for in the next tick
			*/
			debug(...msg) {
				if (!settings.debug) {
					return; // Disabled
				} else if (msg.length == 1 && typeof msg[0] == 'function') { // Given a callback - run it and spew the output
					let payload = msg[0]();
					if (payload instanceof Promise) { // Gave back a promise, resolve it then output
						Promise.resolve(payload) // Given function - resolve it then output (if returns anything)
							.then(payload => payload ?? settings.debugLogger(payload))
					} else { // Gave back a scalar - spew
						settings.debugLogger(payload);
					}
				} else { // Everything else - just spew
					settings.debugLogger(...msg)
				}
			},
		},
	};
...
* @param {Boolean} [options.debugTiming=false] Output data about how long a service took to bootstrap (wait for its `created` function to complete)
* @param {Function} [options.debugLogger] Actual worker function to output data, defaults to `console.log` with a prefix for the service name
...
export default function Service(component, options) {
	...
	if (settings.wrapCreated && component.created) {
		...
		// Handle services that extend others
		let extendCreateCB;
		if (component.extends?.created) { // Extender contains a created handler - replace it
			extendCreateCB = component.extends.created;
			component.extends = { // Shallow copy of extendo
				...component.extends,
				created() {}, // Replace created with dummy as we will already have called it
			};
		}

		component.created = function ServiceCreated() {
			return promisePending = Promise.resolve()
				.then(()=> settings.debugTiming && console.time(`Service: ${settings.name}`))
				.then(()=> settings.onLoad && settings.onLoad.call(this, this))
				.then(()=> extendCreateCB && extendCreateCB.call(this)) // Handle extender created events
				...
				.finally(()=> settings.debugTiming && console.timeEnd(`Service: ${settings.name}`))
		};
