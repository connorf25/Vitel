...
export default function Service(component, options) {
	...
	// Wrap .created() -> ready + promise() (if settings.wrapInit()) {{{
	...
	// Create base VueApp
	let service = createApp(component, props)

	// Relink globalProperties to inherit services (if settings.copyGlobals)
	if (settings.app && settings.copyGlobals) {
		service.config.globalProperties = settings.app.config.globalProperties; // Inherit the same globals
	}
