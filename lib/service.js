...
export function VitelServiceMixin(options) {
	...
	return {
		methods: {
			...
			*
			* @param {Function|*} [message...] Message components to output. If a function is given as the soel argument it is evaluated and the output given as the debug message. Async functions are waited for in the next tick
			*/
			debug(...msg) {
				if (!settings.debug) {
					return; // Disabled
